...
class MusicBot(discord.Client):
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            reply_text = "**%s** songs 노래가 재생목록에 등록되었어요! : %s"
    ...
    async def cmd_하나요(self, channel, author, voice_channel):
        """
        Usage:
            {command_prefix}hanayo

        Call the bot to the summoner's voice channel.
        """

        if not author.voice_channel:
            raise exceptions.CommandError('현재 음성채널에 접속중이 아니에요!')
    ...
    async def cmd_스킵(self, player, channel, author, message, permissions, voice_channel):
        """
        Usage:
            {command_prefix}skip
        ...
        if player.is_stopped:
            raise exceptions.CommandError("지금은 스킵할 수 없어요! 아무것도 재생중이 아니에요!", expire_in=20)

        if not player.current_entry:
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    # print(player.playlist.peek()._waiting_futures[0].__dict__)
                    return Response("다음 노래는 (%s) 네요, 조금만 기다려주세요!" % player.playlist.peek().title)
    ...
    async def cmd_볼륨(self, message, player, new_volume=None):
        ...
        if not new_volume:
            return Response('지금 볼륨은 `%s%%`네요!' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        try:
            ...
            raise exceptions.CommandError('{} 입력이 불가능한 값이에요'.format(new_volume), expire_in=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('볼륨이 %d 에서 %d 로 바뀌었어요!' % (old_volume, new_volume), reply=True, delete_after=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("지금 재생중인 노래는 **%s** added by **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
    ...
    async def cmd_하나요정리(self, message, channel, server, author, search_range=50):
        """
        Usage:
